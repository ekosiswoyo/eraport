<?php 
    if (isset($_POST[simpan])){
        $juml = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM rb_siswa where kode_kelas='$_GET[id]'"));
        for ($ia=1; $ia<=$juml; $ia++){
          $a   = $_POST['a'.$ia];
          $b   = $_POST['b'.$ia];
          $c   = $_POST['c'.$ia];
                              ...
                              while($r=mysqli_fetch_array($tampil)){
                                $des = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM rb_nilai_sikap where kodejdwl='$_GET[jdwl]' AND nis='$r[nis]' AND status='spiritual'"));
                                  echo "<tr>
                                        ...
                                        <td align=center><textarea name='a".$no."' class='form-control' style='width:100%; color:blue' placeholder=' Belum Ada Nilai...' readonly>$des[positif]</textarea></td>
                                        <td align=center><textarea name='b".$no."' class='form-control' style='width:100%; color:blue' placeholder=' Belum Ada Nilai...' readonly>$des[negatif]</textarea></td>
                                        <td align=center><textarea name='c".$no."' class='form-control' style='width:100%; color:blue' placeholder=' Belum Ada Nilai...' readonly>$des[deskripsi]</textarea></td>
                                      </tr>";
                                  $no++;
                                }
