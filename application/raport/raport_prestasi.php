...
if ($_GET[act]==''){ 
    if (isset($_POST[simpan])){
            if ($_POST[status]=='Update'){
              mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE rb_nilai_prestasi SET jenis_kegiatan='$_POST[a]', keterangan='$_POST[b]' where id_nilai_prestasi='$_POST[id]'");
            }else{
              mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO rb_nilai_prestasi VALUES('','$_GET[tahun]','$_POST[nis]','$_GET[kelas]','$_POST[a]','$_POST[b]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
        echo "<script>document.location='index.php?view=prestasi&tahun=$_GET[tahun]&kelas=$_GET[kelas]#$_POST[nis]';</script>";
    }

    if (isset($_GET[delete])){
        mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM rb_nilai_prestasi where id_nilai_prestasi='$_GET[delete]'");
        echo "<script>document.location='index.php?view=prestasi&tahun=$_GET[tahun]&kelas=$_GET[kelas]#$_POST[nis]';</script>";
    }
?> 
            <div class="col-xs-12">  
                ...
                <?php 
                  ...
                  if ($_GET[kelas] != '' AND $_GET[tahun] != ''){
                    ...
                    while($r=mysqli_fetch_array($tampil)){
                      if (isset($_GET[edit])){
                          $e = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM rb_nilai_prestasi where id_nilai_prestasi='$_GET[edit]'"));
                          $name = 'Update';
                      }else{
                          $name = 'Simpan';
                      }
                  ...
                  if ($_GET[nis]==$r[nis]){   
                    ...
                    echo "<form action='index.php?view=prestasi&tahun=$_GET[tahun]&kelas=$_GET[kelas]' method='POST'>
                            ...
                            $pe = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM rb_nilai_prestasi where id_tahun_akademik='$_GET[tahun]' AND nis='$r[nis]' AND kode_kelas='$_GET[kelas]'");
                            while ($n = mysqli_fetch_array($pe)){
                                echo "<tr>
                                        ...
                                        <td>$n[keterangan]</td>
                                        <td width='90px' align=center><a href='index.php?view=prestasi&tahun=".$_GET[tahun]."&kelas=".$_GET[kelas]."&edit=".$n[id_nilai_prestasi]."&nis=".$r[nis]."#$r[nis]' class='btn btn-xs btn-success'><span class='glyphicon glyphicon-edit'></span></a>
                                                        <a href='index.php?view=prestasi&tahun=".$_GET[tahun]."&kelas=".$_GET[kelas]."&delete=".$n[id_nilai_prestasi]."&nis=".$r[nis]."' class='btn btn-xs btn-danger' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></td>
                                      </tr>";
                            }
                      $no++;
                      }
