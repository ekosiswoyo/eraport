...
if ($_GET[act]==''){ 
    if (isset($_POST[simpan])){
            if ($_POST[status]=='Update'){
              ...
              mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO rb_catatan VALUES('','$_POST[nisn]','$_SESSION[kode_kelas]','$_POST[a]')");
            }
        echo "<script>document.location='index.php?view=catatan&kelas=$_SESSION[kode_kelas]#$_POST[nisn]';</script>";
    }
                ...
                <?php 
                  ...
                  if ($_GET[nisn]==$r[nisn]){   
                    ...
                    echo "<form action='index.php?view=catatan&tahun=kelas=$_SESSION[kode_kelas]' method='POST'>
                            ...
                            $pe = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM rb_catatan where nisn='$r[nisn]' AND kode_kelas='$_SESSION[kode_kelas]'");
                            while ($n = mysqli_fetch_array($pe)){
                                echo "<tr>
                                        ...
                                        <td></td>
                                        <td>$n[catatan]</td>
                                        <td width='90px' align=center><a href='index.php?view=catatan&kelas=".$_GET[kelas]."&edit=".$n[id_catatan]."&nisn=".$r[nisn]."#$r[nisn]' class='btn btn-xs btn-success'><span class='glyphicon glyphicon-edit'></span></a>
                                                        <a href='index.php?view=catatan&kelas=".$_GET[kelas]."&delete=".$n[id_catatan]."&nisn=".$r[nisn]."' class='btn btn-xs btn-danger' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></td>
                                      </tr>";
                            }
                      $no++;
                      }
